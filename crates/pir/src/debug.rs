...
pub fn debug_file(db: Db<'_>, file: &PirFile<'_>) -> String {
    let mut out = String::new();
    write_file(&mut out, db, file).expect("formatting to String is infallible");
    out
}

pub fn write_file(out: &mut dyn Write, db: Db<'_>, file: &PirFile<'_>) -> fmt::Result {
    let items = file.items(db);
    writeln!(out, "pir module ({} items)", items.len())?;
    for item in items {
        writeln!(out)?;
...
fn write_item<'db>(out: &mut dyn Write, db: Db<'db>, item: PirItem<'db>) -> fmt::Result {
...
pub fn write_type(out: &mut dyn Write, db: Db<'_>, ty: &Type, prec: u8) -> fmt::Result {
    match ty {
        ...
        Type::Char => out.write_str("char"),
        Type::Array(elem, len) => {
            out.write_char('[')?;
            write_type(out, db, elem, 0)?;
            write!(out, "; {len}]")
        }
        Type::Record(fields) => {
            out.write_char('{')?;
...
fn write_type_arg(out: &mut dyn Write, db: Db<'_>, arg: &TypeArg<'_>) -> fmt::Result {
    match arg {
        TypeArg::Type(ty) => write_type(out, db, ty, 0),
        TypeArg::Index(term) => write!(out, "idx {}", term.debug(db)),
    }
}
